...
function t(name: string, fn: () => boolean) {
  if (fn()) { p++; console.log(`  \x1b[32mPASS\x1b[0m ${name}`) }
  else { f++; console.log(`  \x1b[31mFAIL\x1b[0m ${name}`) }
}
...
else { console.log(`  \x1b[31mFAIL\x1b[0m ${r.error}`); f++ }

console.log('\n--- getStatus ---')
const st = await adapter.getStatus()
if (st.startsWith('HTTP') || st.startsWith('Error')) { console.log(`  \x1b[31mFAIL\x1b[0m ${st}`); f++ }
else t(st, () => true)

console.log(`\n\x1b[1m${p}/${p + f} passed\x1b[0m`)
